...
#############################################################################
##
#F  GROUP_BY_PCGS_FINITE_ORDERS( <pcgs> )
#F  GROUP_BY_PCGS_FINITE_ORDERS( <pcgselts>, <relords>, <decomp> )
##
##  Constructs a pc group (a group defined via a power-conjugate
##  presentation, using a `SingleCollector') isomorphic to the group
##  generated by <pcgs>, resp. by <pcgselts>. Elements of the result are
##  exponent vectors with respect to these generators, so the group
##  produced is finite, of order the product of the relative orders.
##
##  There are two ways to call this function:
##
##  1. With a single argument <pcgs>, a genuine pcgs.
##     The relative orders and the exponent
##     vectors needed for the power and conjugate relations are read off
##     <pcgs> directly, via `RelativeOrders', `ExponentsOfRelativePower'
##     and `ExponentsOfConjugate'.
##
##  2. With three arguments <pcgselts>, <relords>, <decomp>, for use when
##     the element list forms a pcgs mathematically, but not as a GAP object.
##     Here <pcgselts> is a list
##     of elements generating the group, <relords> is the list of their
##     relative orders, and <decomp> is a function that takes an element
##     expressed in terms of <pcgselts> and returns its exponent vector
##     with respect to <pcgselts>.
##
BindGlobal( "GROUP_BY_PCGS_FINITE_ORDERS", function( arg )
local pcgs,ro,decomp,expow,expcon,f,  e,  m,  i,  type,  s,  id,  tmp, j,z;

    pcgs:=arg[1];
    if Length(arg)=1 then
      ro:=RelativeOrders(pcgs);
      if not IsGeneralPcgs(pcgs) then Error("must be pcgs");fi;
      expow:=i->ExponentsOfRelativePower(pcgs,i);
      expcon:=function(j,i)
        return ExponentsOfConjugate(pcgs,j,i);
      end;
      z:=pcgs!.zeroVector;
    elif Length(arg)=3 then
      ro:=arg[2];
      decomp:=arg[3];
      expow:=i->decomp(pcgs[i]^ro[i]);
      expcon:=function(j,i)
        return decomp(pcgs[j]^pcgs[i]);
      end;
      z:=ListWithIdenticalEntries(Length(pcgs),0);
    else
    ...
    for i  in [ 1 .. Length(pcgs) ]  do
        for j  in [ i+1 .. Length(pcgs) ]  do
            #tmp := pcgs[j] ^ pcgs[i];
            tmp := expcon(j,i); #ExponentsOfConjugate(pcgs,j,i);
            if tmp <> id  then
                #tmp := ExponentsOfPcElement( pcgs, tmp );
                tmp := ObjByVector( type, tmp );
